...
void JobInfo::parseCommandline( int argc, char *argv[] )
{
	...
	for( int i = 1; i < argc; i++ )
	{
		...
		switch( param[1] )
		{
...
#ifdef LINUX
				// We just copy name of the serial device.
				comPort = (param + 2);
#else
				if( strlen( param ) != 6 )
			...
			case 't' : // Test mode.
				if( strlen( param ) <= 2 )
					throw new ErrorMsg( "Cannot use -t without the test mode!" );

				/* Copy test mode name string to class variable */
				testModeName.assign( param + 2 );
				testMode = true;
				break;
#endif
...
void JobInfo::doJob()
{
	...
	long scanCOM = -1;
#endif
	SerialPort * com = nullptr;
	AVRProgrammer * prog = nullptr;
	AVRDevice * avr = nullptr;
	string programmerID;
...
#ifdef LINUX
	// If user did not specify serial device return error.
	if (comPort.length() == 0)
	{
		Util.log("Use -c option to specify serial device.");
		return;
	}
#else
...
#ifdef LINUX
	Util.log( "Found " + programmerID + " on " + comPort + "!\r\n" );
#else
...
void JobInfo::enterTestMode()
{
	...
	programmerID = "AVRBOOT";

	Util.log( "Entering test mode...\r\n" );
